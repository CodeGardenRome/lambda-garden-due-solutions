...
/** Add useful set-theoretical operation unuvailable on the scala collection [[Set]] */
object Set_Operations {

  //adds superset check to standard scala [[Set]]
  implicit class Powerset[A](s: Set[A]) {
    def superset_of(that: Set[A]): Boolean = superset_?(s, that)
  }

  /** verifies if [[s1]] is a superset of [[s2]] */
